...
/**
 * The Game class serves as the Model for the Farkle application. This class is
 * responsible for tracking each player, setting high scores, storing the game
 * mode, calculating all scoring, and determining game outcomes.
 * 
...
public class Game {
	 ...
	 * @param controller
	 *            The controller for the game that links the model to the view
	 */
	...
	/**
	 * Calculate the score of a list of integers representing a roll of dice per
	 * the rules of this version of Farkle. This method calculates the scores in
	 ...
	 * 6.6.0 - Three distinct doubles (e.g. 1-1-2-2-3-3) are worth 750 points.
	 * This scoring rule does not include the condition of rolling four of a
	 * kind along with a pair (e.g. 2-2-2-2-3-3 does not satisfy the three
	 * distinct doubles scoring rule).
	 * 6.7.0 - A straight (e.g. 1-2-3-4-5-6), which can only be achieved when
	 * all 6 dice are rolled, is worth 1500 points.
	 * 
	 * @param roll
	 *            A List of Integers representing the roll of dice for which the
	 *            Farkle score will be calculated.
	 * @param checkHeldDice
	 *            A flag that is set to true when the calculated score should be
	 *            zero if any of the dice do not contribute to the score
	...
	public int calculateScore(List<Integer> roll, boolean checkHeldDice) {
		...
		if (roll != null) {
			...
			if (!roll.isEmpty()) {
				...
				// Test to ensure all values are within the range of 1 to 6
				// (inclusive)
				...
				/*
				 * If all values are within the accepted range of 1 to 6
				 * (inclusive) calculate the score
				 */
				if (!incorrectValuePresent) {
					...
					for (int i = 0; i < countedDie.length; i++) {
						...
						if (i >= 1 && i != 4) {
							// If the die appears 1 or 2 times set the flag to
							// true
						 ...
						 * equal 0, it can be deduced that this roll does not
						 * include three pair
						 */
						 ...
						 * the score for every 1. If there are three or more,
						 * add 1000 * 2 ^ (count - 3)
						 */
						case 0:
						 ...
						 * the score for every 5. If there are three or more,
						 * add 500 * 2 ^ (count - 3)
						 */
						case 4:
							if (currentCount > 0 && currentCount < 3) {
						...
						default:
							 ...
							 * 3, add the current die value * 100 * 2 ^ (count -
							 * 3)
							 */
							if (currentCount >= 3) {
								temp = Math.pow(2, (currentCount - 3));
							 ...
							 * value is 6 (index 5) and 6 dice were rolled, it
							 * is confirmed that a straight was rolled, set the
							 ...
							 * die value is 6 (index 5) and 6 dice were rolled,
							 * it is confirmed that 3 pairs was rolled, set the
							 * calculated score to 750
							 */
					...
					/*
					 * If the checkHeldDice flag is true, we need to return 0 if
					 * any of the dice do not contribute to the score.
					 */
					if (checkHeldDice) {

						/*
						 * if the number of counted pairs is less than 3, set
						 * the isThreePair flag to false.
						 */
						if (countOfPairs != 3) {
							isThreePair = false;
						}
						...
						/*
						 * If a 2, 3, 4, or 6 occurs once or twice and the roll
						 * does not contain 3 pair or a straight, set
					...
					/*
					 * Else, at least one die value was outside of the accepted
					 * range, set calculatedScore to 0
					 */
				} else {
					calculatedScore = 0;
				}
			...
			// The list of integers representing the roll was null. set
			// calculatedScore to 0
		} else {
			calculatedScore = 0;
		}
	...
	/**
	 * Calculate the highest possible score for a given roll of the dice, and
	 * details the dice that must be selected to achieve that score.
	 * 
	 * @param dice
	 *            The list of integers representing the roll of dice
	 * @return Object[] array - the first index holds the calculated highest
	 *         possible score for a given roll, and the second index is a list
	 *         of dice that must be selected to achieve that score.
	 */
	public Object[] calculateHighestScore(List<Integer> dice) {
		...
		if ((dice != null) && dice.size() > 0) {
			...
			for (int i = 1; i <= magicNumber; i++) {
				...
				// Add the appropriate die from the roll for each 1 in
				// binaryNumber
				...
				// Compare tempScore to the previously set highest calculated
				// score
	 ...
	 * player's turn.
	 * 
	 * @return int - Game score for the current player when this method was
	 *         called
	 */
	public int bank() {
	 ...
	 * 
	 * @param currentPlayer
	 *            The index of the player to set as current (1 or 2)
	 */
	 ...
	 * 
	 * @param highScore
	 *            integer to save as the high score in preferences
	 */
	...
	/**
	 * Set all the turn scores, roll scores, and game scores for all players in
	 * the Game object to 0, and set the first player as the current player
	 */
	 ...
	 * 
	 * @param playerNumber
	 *            Player's position - NOT AN INDEX (1 or 2)
	 * @param name
	 *            The string used to set the player's name
	 */
	 ...
	 * 
	 * @param playerNumber
	 *            The player for which the game score is sought
	 ...
	 * 
	 * @param playerNumber
	 *            The player for which the name is sought
	 ...
	 * 
	 * @return The player type for the current player (PlayerType.USER or
	 *         PlayerType.COMPUTER)
	 */
	 ...
	 * 
	 * @param playerNumber
	 *            The player for which the type is to be set
	 * @param playerType
	 *            The type to set the chose player to (PlayerType.USER or
	 *            PlayerType.COMPUTER)
	 */
	 ...
	 * 
	 * @return boolean - True if the current turn is a bonus turn, false
	 *         otherwise
	 */
	 ...
	 * 
	 * @param isBonusTurn
	 *            - True if the current turn is a bonus turn, false otherwise
	 */
	...
	public String[] getWinningPlayerInfo() {
		...
		if (this.gameMode != GameMode.MULTIPLAYER) {
			...
			if (player1Score > player2Score) {
				...
				// Else there was a tie, return both players' names and the
				// score
			} else {
